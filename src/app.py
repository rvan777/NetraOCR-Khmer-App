...
class KhmerOCRApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        self.preview_label = ctk.CTkLabel(
            self.preview_frame, text="No Document Loaded", font=ctk.CTkFont(size=16)
        )
        ...
        self.page_label = ctk.CTkLabel(
            self.nav_frame, text="Page 0 / 0", font=ctk.CTkFont(size=14)
        )
        ...
        ctk.CTkLabel(self.bottom_frame, text="Mode:", font=ctk.CTkFont(size=14)).pack(
        ...
        ctk.CTkButton(
            self.bottom_frame, text="Settings", width=100, command=self.open_settings
    ...
    def open_settings(self):
        ...
        ctk.CTkLabel(
            settings_window,
            text="Display & Export Settings",
            font=ctk.CTkFont(weight="bold", size=16),
        ).pack(pady=10)
        ctk.CTkCheckBox(
            settings_window,
            text="Enable Rich Text (Images, Bold)",
            variable=self.rich_text_mode,
        ).pack(anchor="w", padx=20, pady=5)
        ctk.CTkCheckBox(
            settings_window, text="Show Line Numbers", variable=self.show_line_numbers
        ).pack(anchor="w", padx=20, pady=5)
        ctk.CTkCheckBox(
            settings_window,
            text="Show Accuracy Percentage",
            variable=self.show_accuracy,
        ).pack(anchor="w", padx=20, pady=5)
        ctk.CTkLabel(
            settings_window,
            text="*Rich text is slower but includes images",
            font=ctk.CTkFont(size=10),
            text_color="gray",
        ).pack(pady=5)
        ctk.CTkButton(
            settings_window, text="Close", width=80, command=settings_window.destroy
        ).pack(pady=5)
    ...
    def render_preview(self, page_num):
        ...
        self.page_label.configure(
            text=f"Page {page_num + 1} / {self.doc_handler.total_pages}"
        )
        self.prev_btn.configure(state="normal" if page_num > 0 else "disabled")
        self.next_btn.configure(
            state=(
                "normal" if page_num < self.doc_handler.total_pages - 1 else "disabled"
            )
        )
    ...
    def start_ocr(self):
        ...
        threading.Thread(
            ...
            args=(self.ocr_modes[self.ocr_mode.get()], self.rich_text_mode.get()),
            daemon=True,
        ).start()
        ...
        try:
            while True:
                ...
                if msg["type"] == "progress":
                    ...
                    self.markdown_text.append_page(
                        msg["page"], msg["results"], self.rich_text_mode.get()
                    )
    ...
    def _on_finished(self):
        ...
        messagebox.showinfo(
            "Success", f"OCR Processing Complete!\nMode: {self.ocr_mode.get()}"
        )
        if self.worker:
            self.worker.cleanup()
