        ...
        if ext == ".txt":
            with open(path, "w", encoding="utf-8") as f:
                for item in extracted_results:
                    if item["type"] == "text":
                        line = (f"{item['line']}. " if show_lines else "") + item[
                            "text"
                        ]
                        if show_acc:
                            line += f"  [Acc: {item['accuracy']:.1f}%]"
                        f.write(line + "\n")
            ...
            headers = (
                (["Line #"] if show_lines else [])
                + ["Text"]
                + (["Accuracy (%)"] if show_acc else [])
            )
            ws.append(headers)
            for item in extracted_results:
                if item["type"] == "text":
                    row = (
                        ([item["line"]] if show_lines else [])
                        + [item["text"]]
